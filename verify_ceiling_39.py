...
输出:
  data/ceiling_probe_<ts>/pages.ndjson   每页一行: keyword/page/cursor_in/items/found/has_more/cursor_ret
  data/ceiling_probe_<ts>/verdicts.ndjson 每词一行: 逐页摘要 + 判定 + 判据
  data/ceiling_probe_<ts>/probe.log
"""
import asyncio
import json
import os
...
SUSPECT_TERMS = [
    "khoomei", "chinesezither", "solarterms", "chineseink", "活字",
    "throat singing", "northwestchinasong", "木结构", "longsong", "京剧",
    "Li textile", "nanjingbrocade", "kunopera", "urtiinduu", "二十四节气",
    "dongchorus", "silk weaving", "xuanpaper", "篆刻", "剪纸",
    "budaixi", "呼麦", "uyghurmuqam", "tcm", "thangka",
    "印章", "shadow puppetry", "xianwind", "龙泉", "端午",
    "藏医", "mäshräp", "针灸", "longquanceladon", "制茶",
    "24solarterms", "算盘", "farmersdance", "雕版",
]
...
async def main():
    ...
    out = ROOT / "data" / f"ceiling_probe_{ts}"
